...
with patientdata as (
select pd.patient_id, pd.fname, pd.mname, pd.lname, pd.dob, pd.chart_no, pd.sex_type_id, pd.is_active, pa.patient_allergy_id, pa.allergy_master_id, pa.note,
	dense_rank() over (order by fname, pd.patient_id) rowno
	from patient_demographics pd
	left join patient_allergy pa on pa.patient_id=pd.patient_id
	where (pd.is_deleted=false and (pa.is_deleted=false or pa.is_deleted is null)) group by pd.patient_id,pa.patient_allergy_id order by fname
) select * from patientdata where rowno between 1 and 10;
